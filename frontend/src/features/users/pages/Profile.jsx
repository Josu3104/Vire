...
import { getUserById, requestMembershipValidation } from '@/features/users/api/users.api'
...
import { getMyProjects, updateProjectVisibility } from '@/features/projects/api/projects.api'
...
const IconFileText = () => <svg width="1.2em" height="1.2em" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" /><line x1="16" y1="13" x2="8" y2="13" /><line x1="16" y1="17" x2="8" y2="17" /><polyline points="10 9 9 9 8 9" /></svg>
...
const IconBadge = () => <svg width="1.2em" height="1.2em" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="8" r="7" /><polyline points="8.21 13.89 7 23 12 20 17 23 15.79 13.88" /></svg>
const IconMapPin = () => <svg width="1.2em" height="1.2em" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" /><circle cx="12" cy="10" r="3" /></svg>
...
export default function Profile() {
  ...
  useEffect(() => {
    if (isMyProfile) {
      getMyProjects().then(res => setMyProjectsList(res.data || res || [])).catch(console.error)
    }
  }, [isMyProfile])
  ...
  const userProjects = isMyProfile 
    ? myProjectsList.filter(p => p.status === 'publico') 
    : projects.filter((p) => p.authorIds?.includes(profileUser?.id))

  const archivedProjects = isMyProfile ? myProjectsList.filter(p => p.status === 'privado') : []
  const reviewProjects = isMyProfile ? myProjectsList.filter(p => p.status !== 'publico' && p.status !== 'privado') : []

  const handleStatusChange = async (project, newStatus) => {
    try {
      await updateProjectVisibility(project.id, newStatus)
      success(`Proyecto marcado como ${newStatus === 'privado' ? 'Archivado' : 'Público'}.`)
      // Refresh my projects list
      getMyProjects().then(res => setMyProjectsList(res.data || res || [])).catch(console.error)
    } catch (e) {
      console.error(e)
      error('No se pudo actualizar la visibilidad del proyecto.')
    }
  }

  const totalUpvotes = userProjects.reduce((acc, p) => acc + p.upvotes, 0)
  const totalComments = userProjects.reduce((a, p) => a + (p.comments?.filter(c => !c.isHiddenByAuthor).length ?? 0), 0)
  ...
  const handleRequestMembership = async () => {
    try {
      await requestMembershipValidation()
      success('Solicitud de membresía enviada. Será revisada por un administrador.')
      setIsSettingsModalOpen(false)
    } catch (e) {
      console.error(e)
      error('Error al enviar solicitud de membresía.')
    }
  }
  ...
  return (
    <div className={styles.page}>
      ...
      {/* ── Banner ─────────────────────────────────────────────────────── */}
      <div className={styles.bannerWrapper} style={{ backgroundColor: '#4a4a4a' }}>
        ...
        <div className={styles.headerCard}>
          ...
          <div className={styles.nameRow}>
            ...
            {profileUser?.affiliation?.startsWith('IEEE') && (
              <span className={`${styles.affBadge} ${styles.affIEEE}`}><IconBadge /> IEEE</span>
            )}
            {profileUser?.affiliation?.startsWith('CIMEQH') && (
              <span className={`${styles.affBadge} ${styles.affCIMEQH}`}><IconBadge /> CIMEQH</span>
            )}
          </div>
          ...
          {/* Ficha Técnica */}
          <div style={{ marginTop: 24, padding: 16, backgroundColor: 'var(--bg-surface)', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border-default)' }}>
            <h4 style={{ fontSize: 13, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--text-muted)', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 6 }}>
              <IconFileText /> Ficha Técnica
            </h4>
            
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '20px' }}>
              {[
                { icon: <IconMapPin />, label: 'INSTITUCIÓN', value: profileUser?.university || '—' },
                { icon: <IconFileText />, label: 'ESTADO', value: profileUser?.academicStatus || '—' },
                { icon: <IconMapPin />, label: 'SEDE', value: profileUser?.campus || '—' },
                { icon: <IconMapPin />, label: 'CIUDAD', value: profileUser?.city || '—' },
                { icon: <IconBadge />,  label: 'AFILIACIÓN', value: profileUser?.affiliation || 'Ninguna' },
                { icon: <IconVerified />, label: 'CUENTA', value: profileUser?.role === 'member' || profileUser?.role === 'admin' ? 'Validada' : 'Básica' },
              ].map((item, idx) => (
                <div key={idx} style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                  <span style={{ color: 'var(--accent-500)', marginTop: 2 }}>{item.icon}</span>
                  <div>
                    <div style={{ fontSize: 11, color: 'var(--text-muted)', letterSpacing: '0.04em', fontWeight: 600 }}>{item.label}</div>
            ...
            {!canViewFullProfile ? (
              ...
              {isMyProfile && (
                <>
                  ...
                  >
                    <IconFileText /> Archivados ({archivedProjects.length})
                  </button>
                  <button
                  ...
                  >
                    <IconFileText /> En Revisión ({reviewProjects.length})
                  </button>
                </>
              )}
            </div>
              ...
              <div className={styles.archivedContainer}>
                {archivedProjects.length === 0 ? (
                  <div className={styles.emptyState}>
                    ...
                    <p className={styles.emptyTitle}>Sin proyectos archivados</p>
                    <p className={styles.emptyText}>Los proyectos que marques como privados aparecerán aquí.</p>
                  </div>
                ) : (
                  <div className={styles.projectsGrid}>
        ...
        <div className={styles.modalOverlay} onClick={() => setIsSettingsModalOpen(false)}>
          <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
            <div className={styles.modalHeader}>
              <h3 className={styles.modalTitle}>Ajustes de Cuenta</h3>
              <button className={styles.closeBtn} onClick={() => setIsSettingsModalOpen(false)}>×</button>
            </div>
            <div className={styles.modalBody}>
              <p style={{ marginBottom: '16px', color: 'var(--text-muted)' }}>Opciones de configuración de tu cuenta.</p>
              
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px', border: '1px solid var(--border-default)', borderRadius: 'var(--radius-md)' }}>
                <div style={{ textDecoration: isVerified ? 'line-through' : 'none', opacity: isVerified ? 0.6 : 1 }}>
                  <h4 style={{ margin: '0 0 4px', fontSize: '1rem', color: 'var(--text-primary)' }}>Validar Membresía</h4>
                  <p style={{ margin: 0, fontSize: '0.85rem', color: 'var(--text-muted)' }}>Solicita revisión de administrador para validar tu colegiación o membresía.</p>
                </div>
                <button 
                  ...
                  disabled={isVerified}
                >
                  {isVerified ? 'Verificado' : 'Solicitar'}
                </button>
              </div>
            </div>
          </div>
